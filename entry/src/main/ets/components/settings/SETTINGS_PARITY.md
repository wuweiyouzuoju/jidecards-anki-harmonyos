...
| 同步 | 账号、自动同步、立即同步 | 另含自定义服务器 | `同步分组.ets`；简洁版若已配置自建服务器，保留入口以便查看/恢复，不静默改回 AnkiWeb |
| 外观 | 跟随系统/浅色/深色、卡片字号 | 另含颜色主题、支持主题的背景动效、底部/浮动复习布局 | `外观分组.ets`、`布局分组.ets`、`CardTextSizeControl.ets`、`ThemeMotionControl.ets` |
| 控制 | 不展示高级控制入口 | 评分振动、点击区域快速答题 | `ReviewControlsSettings.ets`；后者沿用原实验版生效条件 |
...
| 关于 | 版本、反馈、许可、权益 | 相同 | `../设置面板.ets`、`RedemptionPanel.ets`；识别码按需打开 |
...
- “专属内容”缩窄为“JideCards 主题赠送”；当前兑换白名单由 ThemeCatalog 的主题内容决定，签名/权益协议不变。“应用指纹”对用户称“应用识别码”，放入关于页的主题兑换详情。
- “浮动可拖”改“悬浮，可拖动”，“学习页布局”行改“答题工具栏位置”，“已隐藏牌组”改“首页隐藏的牌组”，说明不会改变复习状态。“学习术语与帮助”目录简化为“学习帮助”。
