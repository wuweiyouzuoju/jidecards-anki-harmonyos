...
export class AutoSyncScheduler {
  ...
  /** 手动请求及其让路后的重试不受自动同步开关影响。 */
  requestManual(): void {
    this.manualPending = true;
    this.request();
  }

  isManualPending(): boolean { return this.manualPending; }

  /** 仅在挂载同步任务时消费；之后的新写入自然进入下一轮。 */
  consume(): void { this.pending = false; this.manualPending = false; }
