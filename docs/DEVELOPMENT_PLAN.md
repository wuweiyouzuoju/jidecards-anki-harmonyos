...
**最高开发优先级是编程 Agent-first：所有功能、修复、重构、测试和文档都要降低编程 Agent 的理解、修改、验证和持续开发成本。**

- 在不削弱 Agent-first 的前提下，保障现有 Anki 牌组直接导入后的学习体验，包括离线数学、物理、化学公式和牌组自带媒体的显示。
- 内容创作重点投入自然语言驱动的应用内 Agent 制卡与改卡，让用户通过描述意图、检查草稿和确认保存完成内容维护。
- 保留现有手动编辑与纠错入口，但不把复杂公式工具栏、所见即所得编辑器或手写排版作为近期重点。
- 渲染能力在学习页和卡片预览中共享，同样服务于导入内容和 Agent 生成内容；不因来源不同使用两套格式。
- 以上优先级不改变应用内 Agent 的发布开关、草稿确认和写入安全边界。
...
1. 持续完善编程 Agent-first 的入口、边界、直接行为测试和验证门禁；新增功能不得
   让关键知识退回历史计划、忽略目录或不可执行的文档约定。
2. 在重新开放应用内 Agent 入口前，完成实体手机、真实 Provider、取消/断网、普通写入
   和高风险写入回归；联网搜索应作为独立开关验收。
3. 为 API 21 最低兼容声明补齐可重复的真机回归记录；若要重新支持 API 12，必须
   先修改构建配置并完成所有高版本 API 的能力检查，不能只改文档。
4. 建立与官方 Anki 26.05 的固定集合差分测试，量化调度、搜索、导入导出和同步
   边界，再决定可对外使用的兼容措辞。
5. 对 tablet/2in1、无障碍、耗电和大集合压力测试保留可追溯的设备与版本信息。
