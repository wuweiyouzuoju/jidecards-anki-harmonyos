...
手动同步与自动同步共用根宿主；设置页发起后返回首页，工具栏下方显示手动任务进度、失败重试或待处理冲突，详情可随时收起。自动传输保持静默，首页延后 3 秒检查；开始学习取消未执行检查，进行中的增量同步通过独立原生取消通道回滚后让路，未同步意图保留，手动重试不受自动同步关闭影响。媒体阶段继续传输；未确认的全量冲突不锁学习，确认覆盖前重新检查方向和端点。已经明确发起的全量替换仍保护数据库直至完成，不能承诺零等待。同步后的 FSRS 提醒延后到空闲首页。
