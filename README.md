...
为配合华为应用市场上架，2.3.6 发布包默认隐藏以下界面入口，但相关源码和自动化测试仍完整保留在仓库中：
...
### 学习时的 AI Agent 改卡（2.3.6 默认隐藏，开发者调试解锁后可用）
...
### AI 制卡与 AI 改卡（2.3.6 默认隐藏，开发者调试解锁后可用）
...
- **联网与来源**：2.3.6 源码保留 Provider 搜索协议与来源展示，但页面当前强制关闭联网搜索；重新开放前不得宣称 Agent 已联网检索。
