...
const names = ['加载下一张卡', '显示答案', '埋藏或暂停当前卡', '评分', 'invalidateCardWork', 'isCurrentRequest', 'studyActivityChanged', 'applyStudyHtml', 'playStudyAudio', 'studyWebAttached', 'aboutToDisappear', '消费待重渲染', '刷新编辑后当前卡', '刷新撤销状态', '撤销上次', '加载完成页信息'];
const methods = names.map(name => {
  const start = source.search(new RegExp(`  (?:private )?(?:async )?${name}\\(`));
  assert.ok(start >= 0);
  return source.slice(start, source.indexOf('\n  }', start) + 4);
});
const Page = new Function('构建卡片HTML', '剥除拼写标记', '提取拼写标记', '原始侧HTML', '媒体基地址', 'CustomTransition', '刷新桌面卡片数据', 'AppStorage', 'SECONDS_PER_DAY', '$r', 'console',
...
function harness() {
  ...
  Object.assign(page, {
    ...
    声音播放器实例:{停止:async()=>events.push('stop'),释放:async()=>events.push('sound release'),播放队列:async paths=>sounds.push(paths)},
    TTS播放器实例:{停止:async()=>{},释放:async()=>events.push('tts release'),播放队列:async()=>{}},
